...
import { useGetService, useUpdateService, useUploadServiceIcon } from '@/tanstack/useServices';
...
export default function EditServiceScreen() {
  ...
  const existing = data?.data?.service ?? data?.data ?? null;
  ...
  useEffect(() => {
    if (existing) {
      ...
      setExistingIcon(existing.icon ?? null);
  ...
  const isBusy = isUpdating || isUploadingIcon;
  ...
  const currentIcon = useMemo(() => {
    ...
    return existingIcon;
  }, [iconRemoved, iconFile, existingIcon]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (iconFile) {
        ...
        await uploadIconAsync({ serviceId: id, formData });
      }

      setInlineStatus({ type: 'success', text: 'Service updated successfully.' });
      setTimeout(() => router.back(), 600);
    } catch (err: any) {
      const message =
        err?.response?.data?.message || err?.message || 'Unable to update service right now.';
