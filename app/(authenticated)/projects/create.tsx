...
export default function CreateProjectScreen() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setInlineStatus({ type: 'success', text: 'Project created successfully.' });
      setTimeout(() => {
        if (createdProject?._id || createdProject?.id) {
          const id = createdProject._id || createdProject.id;
          router.replace(`/(authenticated)/projects/${id}`);
        } else {
      ...
      const message =
        err?.response?.data?.message || err?.message || 'Unable to create project right now.';
            ...
            <View className="flex-row items-center justify-end gap-3">
              ...
              <Pressable onPress={handleSave} disabled={isBusy} className="btn btn-primary min-w-[140px]">
                {isBusy ? (
                  ...
                  <Text className="btn-text btn-text-primary">Create project</Text>
                )}
              </Pressable>
            </View>
          </View>
